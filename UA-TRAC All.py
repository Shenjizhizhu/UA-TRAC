...
from torchvision.models import ResNet34_Weights  # 新增：导入权重枚举类
...
def create_dir(dir_path):
    if not os.path.exists(dir_path):
        os.makedirs(dir_path, exist_ok=True)  # 新增：exist_ok=True，避免重复创建报错
    return dir_path
...
class ResNetFPNYOLOv5(nn.Module):
    def __init__(self, num_classes=1, img_size=640, anchors=None):
        ...
        self.img_size = img_size
        self.num_classes = num_classes
        # 核心修正：使用权重枚举类加载ResNet34，消除弃用警告
        self.backbone = models.resnet34(weights=ResNet34_Weights.IMAGENET1K_V1)  # 等价于原默认行为
        # 若要使用最新权重，可替换为：weights=ResNet34_Weights.DEFAULT
...
class YOLOv5Loss(nn.Module):
    ...
    def match_anchors(self, target_boxes):
        ...
        valid_boxes = target_boxes[valid_mask]  # (N, 4)
        ...
        ciou_scores = torch.stack(ciou_scores, dim=0)  # (N, 9)
        match_idx = torch.argmax(ciou_scores, dim=1)  # (N,)
...
if __name__ == '__main__':
    ...
    # 修正：处理warmup_epochs超过总epochs的情况
    warmup_epochs = min(3, epochs)  # 预热轮数不超过总epochs
